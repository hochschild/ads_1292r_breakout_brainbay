	...
	for(i=3;i<9;i+=9)
	{
		...
		{	
			...
			datac[buff++] = (unsigned char)(Pkt_Counter );
                      ...
                      /*  for(indx = 0; indx < (ADS1299_TRANSMIT_RATE); indx++)
			{
    			   shiftedFCL = shifted_filter + ((signal1_out_buf[buff] -last_sample)<<8);
			   shifted_filter = shiftedFCL - (shiftedFCL>>8);
			   signal2_out_buf[buff] = (shifted_filter+128)>>8;
			   last_sample = signal1_out_buf[buff];	
			   buff = buff + 1;
			}*/
